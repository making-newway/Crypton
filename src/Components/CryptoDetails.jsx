...
const CryptoDetails = () => {
    ...
    const { data: coinHistory, isFetching: historyFetching } = useGetCryptoHistoryQuery({ coinId, period });
    const cryptoDetails = data?.data?.coin;


    if(isFetching || historyFetching) return <LoaderScreen />
    ...
    return (
        <Col className="coin-detail-container">
            ...
            <Select defaultValue="7d" className="select-timeperiod" placeholder="Select Timeperiod" onChange={(value) => setPeriod(value)}>
                {time.map((date) => 
